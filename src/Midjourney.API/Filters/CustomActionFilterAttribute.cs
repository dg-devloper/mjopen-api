    ...
    public class CustomActionFilterAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            else
            {
                ...
                {
                    var error = context.ModelState.Values.FirstOrDefault()?.Errors?.FirstOrDefault()?.ErrorMessage ?? "Parameter exception";

                    Log.Logger.Warning("Parameter exception {@0} - {@1}", context.HttpContext?.Request?.GetUrl() ?? "", error);
